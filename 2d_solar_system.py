...
class Shader:
    def __init__(self):
        ...
        self.lighting = [1, 1, 1]
        self.newMix = [0, 0, 0]
        self.mercuryAxis, self.venusAxis, self.earthAxis, self.marsAxis, self.jupiterAxis, self.saturnAxis, self.uranusAxis, self.neptuneAxis, self.plutoAxis = [], [], [], [], [], [], [], [], []
        self.allAxis = [self.mercuryAxis, self.venusAxis, self.earthAxis, self.marsAxis, self.jupiterAxis, self.saturnAxis, self.uranusAxis, self.neptuneAxis, self.plutoAxis]
        self.sunPos, self.mercuryPos, self.venusPos, self.earthPos, self.marsPos, self.jupiterPos, self.saturnPos, self.uranusPos, self.neptunePos, self.plutoPos = [0, 0, -1], [-5, 0, -1], [-7, 0, -1], [-8, 0, -1], [-10, 0, -1], [-16, 0, -1], [-21, 0, -1], [-24, 0, -1], [-25, 0, -1], [-27, 0, -1]
        ...
        self.celestialPos = [0, 0, -75]
        self.celestialRot = [0, 0, 0]
        self.Shader_List = []
        self.Shader_Axis = []
    ...
    def init(self):
        # the following three lines will seriously task your system, commenting them is advised
        # axis = axis_vertexes()
        # for i in range(len(axis)):
        #     self.Shader_Axis.append([Shaderprogram(axis[i], "shaders6/images/white.jpg", "shaders6/images/f.jpeg"),  [0, 0, 0], self.allRotations[i], [1, 1, 1]])
        x = load_circle()
        # y = load_skymap()
        ...
        for i in range(10):
            self.Shader_List.append([Shaderprogram(x, list_images[i], "shaders6/tex.png"),  self.allPositions[i], self.allRotations[i], self.allScale[i]])
    ...
    def draw_all(self):
        ...
        for i in range(len(self.allPositions)):
            self.Shader_List[i][1] = self.allPositions[i]
            self.Shader_List[i][2] = self.allRotations[i]
        for i in self.Shader_List:
            self.draw(i[0], i[1], i[2], i[3])
        for i in self.Shader_Axis:
            self.draw(i[0], i[1], i[2], i[3], 1)

    def draw(self, shader_file, positions, rotations, scales, choice=0):
    ...
    def revolve_planets(self):
        self.mercuryPos = multiply3by1_matrix(rotateZ(self.Rotational_Degrees[1]), self.mercuryPos, [0, 0, 0])
        self.venusPos = multiply3by1_matrix(rotateZ(self.Rotational_Degrees[2]), self.venusPos, [0, 0, 0])
        self.earthPos = multiply3by1_matrix(rotateZ(self.Rotational_Degrees[3]), self.earthPos, [0, 0, 0])
        self.marsPos = multiply3by1_matrix(rotateZ(self.Rotational_Degrees[4]), self.marsPos, [0, 0, 0])
        self.jupiterPos = multiply3by1_matrix(rotateZ(self.Rotational_Degrees[5]), self.jupiterPos, [0, 0, 0])
        self.saturnPos = multiply3by1_matrix(rotateZ(self.Rotational_Degrees[6]), self.saturnPos, [0, 0, 0])
        self.uranusPos = multiply3by1_matrix(rotateZ(self.Rotational_Degrees[7]), self.uranusPos, [0, 0, 0])
        self.neptunePos = multiply3by1_matrix(rotateZ(self.Rotational_Degrees[8]), self.neptunePos, [0, 0, 0])
        self.plutoPos = multiply3by1_matrix(rotateZ(self.Rotational_Degrees[9]), self.plutoPos, [0, 0, 0])
